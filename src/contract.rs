...
use cosmwasm_std::{
    from_binary, to_binary, Addr, BankMsg, Binary, Coin, Deps, DepsMut, Env, MessageInfo, Response,
    Uint128,
};
...
use crate::msg::{
    EntropyCallbackData, ExecuteMsg, GameResponse, InstantiateMsg, MigrateMsg, QueryMsg, StateResponse,
};
use crate::state::{
    Game, LatestGameIndexResponse, PlayerHistory, RuleSet, State, GAME, IDX, PLAYER_HISTORY, STATE,
};

use crate::helpers::{
    calculate_payout, /* execute_validate_bet ,*/ get_outcome_from_entropy,
    update_game_state_for_loss, update_game_state_for_win, update_player_history_loss,
...
const CONTRACT_NAME: &str = "Whale-of-fortune-v1.0.1";
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Spin { bet_number /* , bet_size */ } => {
            ...
            // Check that only one denom was sent, and that amount is non-zero
            let coin = match one_coin(&info.clone()) {
                Ok(coin) => coin,
        ...
        ExecuteMsg::FreeSpin { bet_number } => {
            ...
            let mut player_history =
                match PLAYER_HISTORY.may_load(deps.storage, game.player.to_string()) {
                    ...
                    Ok(None) => PlayerHistory::new(info.sender.to_string()), // 5 free spins are given to each new player
                    Err(_) => {
        ...
        ExecuteMsg::ReceiveEntropy(data) => {
            ...
            let mut player_history =
                match PLAYER_HISTORY.may_load(deps.storage, game.player.clone().to_string()) {
                    Ok(Some(player_history)) => player_history,
            ...
            if game.is_winner(game.bet_number.into(), outcome.clone()) {
                ...
                let game =
                    update_game_state_for_win(game.clone(), &outcome, calculated_payout.clone());
                GAME.save(deps.storage, idx.into(), &game)?;

                // Update the player's history state
                let player_history = update_player_history_win(
                    &mut player_history,
                ...
                // Update the player's history state
                let player_history =
                    update_player_history_loss(&mut player_history, Uint128::from(game.bet_size));
                PLAYER_HISTORY.save(deps.storage, game.player.to_string(), &player_history)?;
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> Result<Binary, ContractError> {
    ...
    match msg {
        ...
        // If msg is a request for the contract's state
        QueryMsg::GetState {} => {
            // Load the contract state from storage
            let state = STATE.load(deps.storage)?;
            let idx = IDX.load(deps.storage)?;

            // Serialize the state response into a binary format
            // if error occurs during serialization, handle error gracefully and return a custom error
            to_binary(&StateResponse {
                entropy_beacon_addr: state.entropy_beacon_addr,
                // house_bankroll: state.house_bankroll.into(),
                idx: idx.into(),
                contract_version: state.contract_version,
            })
            .map_err(|e| {
                ContractError::QueryError(format!(
                    "Serialization error in QueryMsg::GetState: {}",
                    e
                ))
            })
        }
        ...
        QueryMsg::PlayerHistory { player_addr } => {
            ...
            let player_history = PLAYER_HISTORY
                .load(deps.storage, player_addr.clone().to_string())
            ...
            let win_loss_ratio = if player_history.losses == Uint128::zero() {
                ...
                player_history
                    .wins
                    .checked_div(player_history.losses)
                    .unwrap()
            };
        ...
        QueryMsg::LatestGameIndex {} => {
            ...
            // if error occurs during serialization, handle error gracefully and return a custom error
            to_binary(&LatestGameIndexResponse { idx: idx.into() }).map_err(|e| {
                ContractError::QueryError(format!(
