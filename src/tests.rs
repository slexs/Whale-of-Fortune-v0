...
pub mod tests {
    ...
    use crate::helpers::{
        build_response, calculate_beacon_fee, calculate_payout, get_bankroll_balance,
        get_outcome_from_entropy, load_player_history_or_create_new, query_leaderboard,
        save_game_state, update_game_and_player_history, update_game_state_for_win,
        update_leaderboard, update_player_history_loss, update_player_history_win,
        validate_bet_amount, validate_bet_number, validate_bet_vs_bankroll, validate_denom,
        validate_funds_sent, validate_sent_amount_to_cover_fee, verify_callback_sender,
    };
    use crate::msg::{
        EntropyCallbackData, ExecuteMsg, GameResponse, InstantiateMsg, MigrateMsg, QueryMsg,
    };
    use crate::state::{
        Game, LatestGameIndexResponse, LeaderBoardEntry, PlayerHistory, RuleSet, State, GAME, IDX,
        PLAYER_HISTORY, STATE,
    };
    ...
    use cosmwasm_std::{
        coins, from_binary, from_slice, to_binary, to_vec, Addr, Api, Coin, CosmosMsg, Deps,
        DepsMut, Empty, MemoryStorage, MessageInfo, Order, Querier, QuerierWrapper, Response,
        StdError, Storage, Uint128, WasmMsg, WasmQuery,
    };
    ...
    fn test_proper_initialization() {
        ...
        assert_eq!(
            res.attributes[1],
            (
                "entropy_beacon_addr".to_string(),
                msg.entropy_beacon_addr.to_string()
            )
        );
    ...
    fn test_spin() {
        ...
        let state = State {
            admin: info.sender.clone(),
            entropy_beacon_addr: Addr::unchecked(
        ...
        let beacon_fee =
            calculate_beacon_fee(&deps.as_mut(), &info.sender.to_string(), callback_gas_limit)
                .unwrap();
        ...
        assert_eq!(
            entropy_request.funds,
            vec![Coin::new(beacon_fee.into(), "ukuji".to_string())]
        );
        ...
        assert_eq!(submsg.msg, CosmosMsg::Wasm(expected_entropy_callback));
    }
    ...
    fn test_free_spin() {
        ...
        PLAYER_HISTORY
            .save(&mut deps.storage, info.sender.to_string(), &player_history)
            .unwrap();
    ...
    fn test_recieve_entropy() {
        ...
        let game = Game::new_game(&info.sender.to_string(), idx.into(), 6u128, 10u128);
    ...
    fn test_calculated_payout_outcome_out_of_range() {
        ...
        let calculated_payout = calculate_payout(Uint128::new(1), 7u8);
    ...
    fn test_get_outcome_from_entropy_outcome_4() {
        ...
        let sum_0123 = (rule_set.zero.u128()
            + rule_set.one.u128()
            + rule_set.two.u128()
            + rule_set.three.u128()) as u32;
    ...
    fn test_get_outcome_from_entropy_outcome_5() {
        ...
        let sum_01234 = (rule_set.zero.u128()
            + rule_set.one.u128()
            + rule_set.two.u128()
            + rule_set.three.u128()
            + rule_set.four.u128()) as u32;
    ...
    fn test_get_outcome_from_entropy_outcome_6() {
        ...
        let sum_012345 = (rule_set.zero.u128()
            + rule_set.one.u128()
            + rule_set.two.u128()
            + rule_set.three.u128()
            + rule_set.four.u128()
            + rule_set.five.u128()) as u32;
    ...
    fn test_player_history() {
        ...
        let player_history_result = PLAYER_HISTORY
        ...
        let mut player_history = PLAYER_HISTORY
            .load(&deps.storage, info.sender.clone().to_string())
            .unwrap();
    ...
    fn test_calculate_payout() {
        assert_eq!(calculate_payout(Uint128::new(1), 0), Uint128::new(1));
        assert_eq!(calculate_payout(Uint128::new(1), 1), Uint128::new(3));
        assert_eq!(calculate_payout(Uint128::new(1), 2), Uint128::new(5));
        assert_eq!(calculate_payout(Uint128::new(1), 3), Uint128::new(10));
        assert_eq!(calculate_payout(Uint128::new(1), 4), Uint128::new(20));
        assert_eq!(calculate_payout(Uint128::new(1), 5), Uint128::new(45));
        assert_eq!(calculate_payout(Uint128::new(1), 6), Uint128::new(45));

        let payout = calculate_payout(Uint128::new(100), 2);
    ...
    fn test_load_player_history_or_create_new_ok_none() {
        ...
        let deps: DepsMut<Empty> = DepsMut {
            // Update here
            api: &api,
            storage: &mut storage,
            querier: querier,
        ...
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), PlayerHistory::new(sender.to_string()));
    }

    #[test]
    fn test_verify_callback_sender_caller_error() {
        let result = verify_callback_sender(
            &"wrong_caller".to_string(),
            &"beacon".to_string(),
            &"requester".to_string(),
            &"trusted".to_string(),
        );
        assert!(matches!(
            result,
            Err(ContractError::CallBackCallerError { .. })
        ));
    }
    ...
    fn test_verify_callback_sender_requester_error() {
        let result = verify_callback_sender(
            &"beacon".to_string(),
            &"beacon".to_string(),
            &"wrong_requester".to_string(),
            &"trusted".to_string(),
        );
        assert!(matches!(
            result,
            Err(ContractError::EntropyRequestError { .. })
        ));
    }
    ...
    fn test_query_player_history() {
        ...
        PLAYER_HISTORY
            .save(&mut deps.storage, player_addr.to_string(), &player_history)
            .unwrap();

        // Call the query function for QueryMsg::PlayerHistory
        let query_msg = QueryMsg::PlayerHistory {
        ...
        assert_eq!(
            player_history_response.player_address,
            player_addr.to_string()
        );
        assert_eq!(
            player_history_response.games_played,
            player_history.games_played
        );
        assert_eq!(player_history_response.wins, player_history.wins);
        assert_eq!(player_history_response.losses, player_history.losses);
        assert_eq!(
            player_history_response.total_coins_spent,
            player_history.total_coins_spent
        );
        assert_eq!(
            player_history_response.total_coins_won,
            player_history.total_coins_won
        );
        assert_eq!(
            player_history_response.free_spins,
            player_history.free_spins
        );
    }
    ...
    fn test_query_latest_game_index_unable_to_load() {
        ...
        let mut none_storage = NoneStorage {
    ...
    #[test]
    fn test_admin_execute_change_entropy_addr() {
        // Set up mock data
        let mut deps = mock_dependencies();
        let env = mock_env();
        let admin = String::from("admin");
        let new_addr = Addr::unchecked("new_addr");

        // Set up the state with the current admin address
        let mut state = State {
            entropy_beacon_addr: Addr::unchecked("old_addr"),
        ...
        // Execute the AdminExecuteChangeEntropyAddr message with the correct admin address
        let info = mock_info(admin.as_str(), &[]);
        let msg = ExecuteMsg::AdminExecuteChangeEntropyAddr {
            new_addr: new_addr.clone(),
        };
        let res = execute(deps.as_mut(), env.clone(), info.clone(), msg.clone()).unwrap();

        // Check that the new entropy address was saved to state
        let state = STATE.load(&deps.storage).unwrap();
        assert_eq!(state.entropy_beacon_addr, new_addr);

        // Check that the response is as expected
        assert_eq!(
            res.attributes[0],
            ("action".to_string(), "change_entropy_addr".to_string())
        );
        assert_eq!(
            res.attributes[1],
            ("new_addr".to_string(), new_addr.to_string())
        );

        // Execute the AdminExecuteChangeEntropyAddr message with an incorrect admin address
        ...
        // Check that the function returns an UnauthorizedNotAdmin error
        match res {
            Err(ContractError::UnauthorizedNotAdmin { addr }) => {
    ...
    #[test]
    fn test_admin_execute_change_free_spin_threshold() {
        // Set up mock data
        let mut deps = mock_dependencies();
        let env = mock_env();
        let admin = String::from("admin");
        let new_threshold = Uint128::new(10);

        // Set up the state with the current admin address and free spin threshold
        let mut state = State {
            entropy_beacon_addr: Addr::unchecked("entropy"),
            admin: Addr::unchecked(admin.clone()),
            free_spin_threshold: Uint128::new(5),
            house_bankroll: Coin {
                denom: "ukuji".to_string(),
        ...
        // Execute the AdminExecuteChangeFreeSpinThreshold message with the correct admin address
        let info = mock_info(admin.as_str(), &[]);
        let msg = ExecuteMsg::AdminExecuteChangeFreeSpinThreshold {
            new_threshold: new_threshold.clone(),
        };
        let res = execute(deps.as_mut(), env.clone(), info.clone(), msg.clone()).unwrap();

        // Check that the new free spin threshold was saved to state
        let state = STATE.load(&deps.storage).unwrap();
        assert_eq!(state.free_spin_threshold, new_threshold);
        ...
        assert_eq!(
            ...
            (
                "action".to_string(),
                "change_free_spin_threshold".to_string()
            )
        );
        assert_eq!(
            res.attributes[1],
            ("new_threshold".to_string(), new_threshold.to_string())
        );

        // Execute the AdminExecuteChangeFreeSpinThreshold message with an incorrect admin address
        let info = mock_info("random", &[]);
        let msg = ExecuteMsg::AdminExecuteChangeFreeSpinThreshold {
            new_threshold: Uint128::new(10),
        };
        let res = execute(deps.as_mut(), env.clone(), info.clone(), msg.clone());
