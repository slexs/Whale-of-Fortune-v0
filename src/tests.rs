...
// #[test]
// fn test_increment_game_id() {
//     let mut deps = mock_dependencies();
    
//     let idx = Uint128::zero();
//     println!("idx: {:?}", idx);

//     IDX.save(&mut deps.storage, &(idx + Uint128::from(1u128))).unwrap();

//     // load idx
//     let idx = IDX.load(&deps.storage).unwrap();
//     assert_eq!(idx, Uint128::from(1u128));

//     IDX.save(&mut deps.storage, &(idx + Uint128::from(1u128))).unwrap();
    
//     // load idx 
//     let idx = IDX.load(&deps.storage).unwrap();
//     assert_eq!(idx, Uint128::from(2u128));
